<template>
  <div class="tool-panel">
    ...
    <el-alert type="info" show-icon :closable="false" class="mb-3">
      <template #title>
        登录 www.xiaohongshu.com → F12 → 网络 → 复制完整 Cookie（需含
        <code>a1</code>、<code>web_session</code>）。按垂类搜索爆款视频笔记，默认仅视频、按热度排序，每类 5 条、热度 ≥2 万。
        服务端需 Python3 + <code>pip install xhshow</code>。下载请点 <strong>「下载」</strong>（服务端代下，直链在浏览器里常会 403）。
        若出现 HTTP 461，请先在浏览器完成人机验证后重试。
      </template>
    </el-alert>
    ...
    <el-form label-width="108px" class="form-block">
      ...
      <el-form-item label="热度下限">
        ...
        <span class="hint inline">推荐 20000；小红书搜索页多为点赞，建议用「自动」或「仅点赞」</span>
      </el-form-item>
    ...
    <div v-if="results.length" class="results">
      <div v-for="block in results" :key="block.categoryId" class="cat-block">
        ...
        <el-table v-if="block.videos?.length" :data="block.videos" stripe size="small">
          ...
          <el-table-column label="笔记页" width="64">
            <template #default="{ row }">
            ...
            <template #default="{ row }">
              <template v-if="row.downloadUrl">
                <el-button
                  size="small"
                  type="primary"
                  :loading="downloadingId === row.noteId"
...
<script setup>
  ...
  import {
    crawlXhsIndustryVideos,
    getXhsOfficialCategories,
  ...
  defineOptions({ name: 'XhsVideoCrawlTool' })

  const STORAGE_KEY = 'dnf1688_xhs_cookie'

  const cookie = ref('')
  const categories = ref([])
  const selectedIds = ref([1, 2, 3, 4, 5, 7])
  ...
  const downloadVideo = async (row) => {
    ...
    try {
      ...
      const safe = (row.title || row.noteId || 'xhs-video').replace(/[<>:"/\\|?*]/g, '_').slice(0, 60)
  ...
  const sourceLabel = (s) => (s === 'search' ? '热度搜索' : s || '搜索')
  ...
  const verifyCookie = async () => {
    ...
    verifyInfo.value = null
    try {
      const res = await verifyXhsCookie({ cookie: cookie.value.trim() })
  ...
  onMounted(async () => {
    ...
    try {
      const res = await getXhsOfficialCategories()
  ...
  const run = async () => {
    ...
    saveCookieLocal()
    try {
      const res = await crawlXhsIndustryVideos({
        cookie: cookie.value.trim(),
      ...
      if (ok === 0) {
        ElMessage.warning('未抓到数据：先检测 Cookie，确认服务端已 pip install xhshow')
      } else {
  ...
  const exportJson = () => {
    ...
    const a = document.createElement('a')
    a.href = URL.createObjectURL(blob)
    a.download = `xhs-crawl-${Date.now()}.json`
    a.click()
    URL.revokeObjectURL(a.href)
  }
</script>
